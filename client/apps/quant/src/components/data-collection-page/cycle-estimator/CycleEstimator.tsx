...
export default function CycleEstimator(props: IProps) {
	...
	return (
		<div className={ 'cycle-estimator ' + props.className }>
			<h3
				id={ lowercaseGamemode + '-cycle-label' }
				className="objective-title"
			>
				Current { props.gamemode } Cycle
			</h3>
			...
			<ToggleButtonGroup
				...
				exclusive
				onChange={ (event, newValue) => setSelectedAccuracy(newValue) }
			>
				{
			...
			<Button color="primary" disabled={ !isValid }>Add</Button>
			<div className="cycle-count"></div>
		</div>
	);
}
